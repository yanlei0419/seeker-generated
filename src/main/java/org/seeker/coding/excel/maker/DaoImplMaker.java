...
public class DaoImplMaker {
	...
	private static List<MyColumn> relateList = new ArrayList<MyColumn>();// 用来保存有关联信息的列
	
	private static StringBuffer sb_share = new StringBuffer();// list count get可共享的输出代码
	private static StringBuffer sb_share_po = new StringBuffer();// list count get可共享的输出代码
	private static StringBuffer sb_share_search = new StringBuffer();// list count
	
	
	private static Map<String, MyColumn> columnMap ;// 获得字段信息
	private static Set<Map.Entry<String, MyColumn>> set ;
	...
	private static StringBuffer createList(MyTable table) {
		...
		for (MyColumn column : relateList) {
			...
			if (arr.length == 4) {
				...
				sb_share_po.append("				\"        t" + index++ + "." + arr[3] + " " + arr[0] + douhao + "\"+" + "\r\n");
			}
		}
